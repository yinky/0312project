...
def train(model, criterion, optimizer, epoch):
    ...
    for i in range(len(train_labeled_loader)):
        ...
        inputs_u_s=inputs_u_w# inputs_u_w为弱增强, inputs_u_s为强增强，暂且不做增强
        if torch.cuda.is_available():#如果有GPU，则需把要计算的数据放在GPU上
            inputs_s, targets_s = inputs_s.cuda(), targets_s.cuda()
            inputs_u_w, inputs_u_s = inputs_u_w.cuda(), inputs_u_s.cuda()
        inputs = interleave(torch.cat((inputs_s, inputs_u_w, inputs_u_s)), 2*7+1)#有标记:弱增强:强增强1:7:7
        logits = model(inputs)
        logits = de_interleave(logits, 2*7+1)
        #targets_s.size()[0]为当前batch的大小，注意最后一组数据不一定能装满
    ...
    unlabeled = iter(train_unlabeled_loader)
    for i in range(len(train_labeled_loader)):
        inputs_s, targets_s = labeled.next()#取下一个batch
        inputs_u, _ = unlabeled.next()

        if torch.cuda.is_available():#如果有GPU，就把要计算的数据放在GPU上
            inputs_s, targets_s = inputs_s.cuda(), targets_s.cuda()
            inputs_u = inputs_u.cuda()

        logits_s = model(inputs_s)
        # logits_s = torch.nn.functional.softmax(logits_s, dim=1)
        Ls = criterion(logits_s, targets_s)#求有标签数据的loss函数的值，图片经过模型输出的分布和答案分布的差异

        #tensor中有个属性叫梯度，默认参与计算。tensor是数据结构/类
        #with torch.no_grad():#此处不在训练，去梯度, 不去也行
        ...
        mask = p_max.ge(0.95)#置信度高于0.95的图片，对应为true，否则为false [512]
        ...
        optimizer.zero_grad()#每个tensor中有梯度，反向传播之前需要先清零
        loss.backward()#loss是一个tensor，默认是相加
        optimizer.step()
    '''
